...
export default class MaktabTadbirlari extends Component {
  ...
  getEvents = () => {
    ...
    axios.get(`${url}/school-by-admin/${v}`).then((res) => {
      this.setState({ data: [Number(res.data.params[0]),Number(res.data.params[0])], });
    });
    getEvents()
      .then((res) => {
              ...
              <div className="mapsr">
                ...
                <YMaps key={"uz_UZ"} query={{ lang: "uz_UZ" }}>
                  <Map
                    ...
                    // style={{marginLeft:"10%"}}
                    state={{
                      center: this.state.data!==null && this.state.data.length===2?this.state.data:[41.552486, 60.620890],
                      zoom: 13,
                    }}
                  >
                    <Clusterer
